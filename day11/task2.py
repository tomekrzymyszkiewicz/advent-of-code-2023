...
def count_paths_to_other_nodes(
    galaxy_map: list[list[str]], node_i: int, node_j: int,empty_rows_set:list, empty_columns_set:set
) -> int:
    paths_length = 0
    for i in range(node_i, len(galaxy_map)):
        for j in range(len(galaxy_map[0])):
            if galaxy_map[i][j] == "#" and not (node_i == i and node_j == j):
                ...
                paths_length += abs(i - node_i) + abs(j - node_j) + (empty_space_dist-1)*(count_empty_rows + count_empty_columns)
